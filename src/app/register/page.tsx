  ...
  if (sent) {
    return (
      <div className="min-h-screen flex items-center justify-center p-4 bg-slate-900 text-slate-50">
        <div className="w-full max-w-md bg-slate-800 rounded-2xl p-8 border border-slate-700 text-center shadow-xl">
          ...
          <a href={buildWhatsAppUrl()} target="_blank" rel="noreferrer" className="inline-block mt-5 text-green-400 hover:text-green-300 font-semibold text-sm">
            Não abriu? Clique aqui para falar no WhatsApp
          </a>
          <div className="mt-6">
            <Link href="/login" className="text-slate-400 hover:text-white text-sm">← Voltar para o login</Link>
          </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-slate-900 text-slate-50">
      <div className="w-full max-w-md bg-slate-800 rounded-2xl shadow-xl p-6 sm:p-8 border border-slate-700">
        <div className="flex flex-col items-center mb-5">
          ...
          <h2 className="text-2xl font-bold text-white">Solicitar Cadastro</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1.5">Tenho interesse em</label>
            <select
              value={interesse}
              onChange={(e) => setInteresse(e.target.value)}
              className="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-green-500 transition-colors"
            >
              {INTERESSES.map((i) => <option key={i} value={i}>{i}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1.5">Mensagem (opcional)</label>
            <textarea
              value={message}
